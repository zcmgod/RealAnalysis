...
\begin{example}
    讨论函数列
    \[
        f_n(x) = \frac{1}{2n}\ln \left(1+n^2x^2\right), n = 1,2,\cdots
        \]
    与
    \[
        f'_n(x) = \frac{nx}{1+n^2x^2}, n = 1,2,\cdots
        \]
    导函数列$\displaystyle \left\{f'_n(x)\right\}$在$[0,1]$上不一致收敛，但是交换极限与导数
    法则成立。说明：一致收敛是极限运算与求导运算交换的充分条件，
    不是必要条件。
\end{example}

现在来讨论定义在区间$[a,b]$上的函数项级数
\begin{equation}
...
\begin{theorem}{\rm (连续性)}
    若函数项级数$\displaystyle \sum u_n(x)$ 在区间$[a,b]$ 上一致收敛，且
    每一项都连续，则和函数在$[a,b]$上也连续。
\end{theorem}
这个定理指出：在一致收敛条件下，求和运算与极限运算可以交换顺序：
\begin{equation}
    \sum \left(\lim_{x \to x_0}u_n(x)\right) = \lim_{x \to x_0}\left(\sum u_n(x)\right)
    \label{eq:eq13}
\end{equation}

\begin{theorem}{\rm (逐项求积)}
    若函数项级数$\displaystyle \sum u_n(x)$ 在区间$[a,b]$ 上一致收敛，且
    每一项都连续，则
    \[
        \sum \int_a^bu_n(x)\, {\rm{d}} x = \int_a^b\sum u_n(x) \, \mathrm{d}x.
        \]
\end{theorem}
\begin{theorem}{\rm (逐项求积)}
    若函数项级数$\displaystyle \sum u_n(x)$ 在区间$[a,b]$ 上每一项都有连续
    导数$x_0 \in [a,b]$为$\displaystyle \sum u_n(x)$的收敛点，且
    $\displaystyle \sum u'_n(x)$在$[a,b]$上一致收敛，则：
    \[
        \sum u'_n(x) = \left(\sum u_n(x)\right)'
        \]
\end{theorem}
\begin{example}
    设
    \[
        u_n(x) = \frac{1}{n^3} \ln \left(1+n^2x^2\right), n = 1,2,\cdots,
        \]
    证明函数项级数$\displaystyle \sum u_n(x)$在$[0,1]$上一致收敛，
    并讨论其和函数在 $[0,1]$上的连续性、可积性和可畏性。
\end{example}

\begin{example}
    证明：函数$\displaystyle \zeta(x) = \sum_{n=1}^{\infty} \frac{1}{n^x}$
    在$(1, +\infty)$上有连续的各阶导函数。
\end{example}
